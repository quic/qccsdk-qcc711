...
/*  
  ...
  ==================================================================== 
  $Header: //depot/prj/qca/rhino/main/fpga/release/header_files/rhino_rgm_CL2015922/HALhwio.h#1 $ $DateTime: 2013/05/30 11:55:02 $ $Author: chinghwa $
  ====================================================================
*/ 
 ...
 */
#define __inp(port)         (*((volatile uint8_t *) (port)))
#define __inpw(port)        (*((volatile uint16_t *) (port)))
#define __inpdw(port)       (*((volatile uint32_t *) (port)))
#define __outp(port, val)   (*((volatile uint8_t *) (port)) = ((uint8_t) (val)))
#define __outpw(port, val)  (*((volatile uint16_t *) (port)) = ((uint16_t) (val)))
#define __outpdw(port, val) (*((volatile uint32_t *) (port)) = ((uint32_t) (val)))
...
#define out_byte_masked(io, mask, val, shadow)  \
  HWIO_INTLOCK();    \
  shadow = (shadow & (uint8_t)(~(mask))) | ((uint8_t)((val) & (mask))); \
  out_byte( io, shadow); \
  HWIO_INTFREE()
#define out_byte_masked_ns(io, mask, val, current_reg_content)  \
  out_byte( io, ((current_reg_content & (uint8_t)(~(mask))) | \
                ((uint8_t)((val) & (mask)))) )
...
#define out_word_masked(io, mask, val, shadow)  \
  HWIO_INTLOCK( ); \
  shadow = (shadow & (uint16_t)(~(mask))) |  ((uint16_t)((val) & (mask))); \
  out_word( io, shadow); \
  HWIO_INTFREE( )
...
#define out_dword_masked(io, mask, val, shadow)  \
   HWIO_INTLOCK(); \
   shadow = (shadow & (uint32_t)(~(mask))) | ((uint32_t)((val) & (mask))); \
   out_dword( io, shadow); \
   HWIO_INTFREE()
#define out_dword_masked_ns(io, mask, val, current_reg_content) \
  out_dword( io, ((current_reg_content & (uint32_t)(~(mask))) | \
                 ((uint32_t)((val) & (mask)))) )
